...
HA-adapted: pure async (httpx, already a Home Assistant dependency and used by
status_checker). Nothing blocking runs on the event loop.
"""
import asyncio
import logging
import re
...
async def probe_open_ports(
    ...
    """
    Probe every open port for HTTP signals (probe all, match after).
