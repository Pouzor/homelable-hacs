...
export function ProxmoxImportModal({ open, onClose, onAddToCanvas, onPendingImported }: ProxmoxImportModalProps) {
  ...
  // Prefill the connection form from the integration options (non-secret) each
  // time the modal opens, so a configured host/port/TLS is ready to import and
  // a stored token can be reused without re-typing it.
  useEffect(() => {
    if (!open) return
    let cancelled = false
    proxmoxApi
  ...
  // WS commands reject with a thrown Error whose message carries the reason.
